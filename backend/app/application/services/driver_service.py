...
from app.domain.exceptions.driver import DriverAlreadyExistsException, DriverNotFoundException
...
class DriverService:
    ...
    async def create_driver(self, dto: DriverCreate, current_user_id: uuid.UUID) -> Driver:
        ...
        Raises DriverAlreadyExistsException if employee code or license is already in use.
        """
    ...
    async def delete_driver(self, driver_id: uuid.UUID, current_user_id: uuid.UUID) -> bool:
        ...
        Raises DriverNotFoundException.
        """
        driver = await self.repository.get_by_id(driver_id)
        if not driver:
            raise DriverNotFoundException(driver_id)
        ...
        return await self.repository.delete(driver_id, soft=True)

    async def paginate_drivers(self, page: int, size: int) -> tuple[Sequence[Driver], int]:
        """
        Fetches a paginated list of drivers.
        """
        return await self.repository.paginate(page, size)
