...
@router.delete("/{session_id}", response_model=APIResponse[None], dependencies=[Depends(PermissionChecker("auth:update"))])
...
@router.delete("", response_model=APIResponse[None], dependencies=[Depends(PermissionChecker("auth:update"))])
async def revoke_all_sessions(
    current_user: User = Depends(get_current_active_user),
    service: SessionService = Depends(get_session_service)
):
    await service.revoke_all_sessions(current_user.id)
    return APIResponse(success=True, message="All sessions revoked successfully.", data=None)
