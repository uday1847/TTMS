...
export function DashboardLayout() {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    `px-4 py-2 rounded-md text-sm font-medium ${isActive ? 'bg-accent text-accent-foreground' : 'hover:bg-accent hover:text-accent-foreground'
    }`
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 bg-card border-r border-border hidden md:flex flex-col">
        ...
        <nav className="flex-1 p-4 flex flex-col gap-2">
          ...
          <PermissionGuard permission="parties:read">
            <NavLink to="/parties" className={navLinkClass}>
              Parties
            </NavLink>
          </PermissionGuard>
